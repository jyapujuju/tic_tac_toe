$(document).ready(function() {
  ...
  function reload_div(){
    ...
    $("#9").val('');
    $("#9").text('-');
    $("#turn").text('P1 turn').css("color","black");
    $("#score").empty();
  }
  function logic(button_val){
    $(button_val).val() === "P1" ? P1_score++ : P2_score++;
    $("#result_p1").text("P1 win " + P1_score + " game");
    $("#result_p2").text("P2 win " + P2_score + " game");
    // $("#score").text($("#1").val() + ' win');
    $("#turn").css("color","Lime").text($(button_val).val() + ' win');
  }
